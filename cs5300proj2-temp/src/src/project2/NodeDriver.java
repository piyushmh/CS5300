...
public class NodeDriver {
	...
	/*
	 * There is a pre-processed file called Preprocess_76.txt made by a Python
	 * Script that Matthew did that rejects 0.998754% of the entries in
	 * edges.txt. fromNetID = 0.76 rejectMin = 0.99 * fromNetID; rejectLimit =
	 * rejectMin + 0.01;
	 * 
	 * Reject Min: 0.752400 Reject Limit: 0.762400
	 */
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < NUM_ITERATIONS; i++) {
			...
			job.setJarByClass(src.project2.NodeDriver.class);

			// Set Mapper and Reducer class
			job.setMapperClass(src.project2.LeMapper.class);
			job.setReducerClass(src.project2.LeReducer.class);
